...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const plusMidRef = useRef<HTMLSpanElement | null>(null);
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (!panel || !plusH || !plusMid || !plusV || !icon || !textInner) return;
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    if (opening) {
      spinTweenRef.current = gsap
        ...
        .to(top, { y: 0, rotate: 45, duration: 0.35, ease: 'power3.out' }, 0)
        .to(mid, { opacity: 0, scaleX: 0, duration: 0.2, ease: 'power2.out' }, 0)
        .to(bot, { y: 0, rotate: -45, duration: 0.35, ease: 'power3.out' }, 0);
    } else {
      spinTweenRef.current = gsap
        .timeline({ overwrite: 'auto' })
        ...
        <div className="sm-panel-inner">
          ...
          {displaySocials && socialItems && socialItems.length > 0 && (
            <div className="sm-socials" aria-label="Social links">
              <h3 className="sm-socials-title">Contacts</h3>
