...
export const useGameLogic = (seed?: number, userCode?: string) => {
  ...
  const getReversedImageName = useCallback((bonusType: BonusType) => {
    return `${bonusType}-negative`;
  }, []);
  ...
  const createObstacles = useCallback(
    (existingObstacles: Obstacle[]): Obstacle[] => {
  ...
  const updateObstacles = useCallback((gameState: GameState) => {
